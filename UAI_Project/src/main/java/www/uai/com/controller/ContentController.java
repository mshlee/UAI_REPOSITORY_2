...
public class ContentController {
	...
	public String noticeBoardPage(Model model, String searchWord, String searchTarget, String b_type) { // 여러 개가 넘어올 경우
																										// VO를 또 생성하여
																										// 받아올 수 있음. 싫으면
																										// ContentVO에
																										// string
		...
		b_type = "0";
		ArrayList<BoardDataVO> dataList = contentService.getContentsList(searchWord, searchTarget, b_type); // getContentsList
																											// 내부에서 검색어
																											// 있음/없음 분기를
																											// 나누던가, 아니면
																											// 또다른 메서드를
	...
	public String reviewBoardPage(Model model, String searchWord, String searchTarget, String b_type) { // 여러 개가 넘어올 경우
																										// VO를 또 생성하여
																										// 받아올 수 있음. 싫으면
																										// ContentVO에
																										// string
	...
	public String qnaBoardPage(Model model, String searchWord, String searchTarget, String b_type) { // 여러 개가 넘어올 경우 VO를
																										// 또 생성하여 받아올 수
																										// 있음. 싫으면
																										// ContentVO에
																										// string
	...
	public String writeContentAction(MultipartFile[] files, ContentDataVO contentDataVO, HttpServletRequest request) { // request는
																														// 위치
																														// 받아오기
																														// 용도
	...
	@RequestMapping("/confirmPasswordPage")
	public String confirmPasswordPage(String b_idx, Model model) {
		model.addAttribute(b_idx);

		return "writePasswordPage";
	}
	...
	public String readQnAContentPage(ContentDataVO contentDataVO, Model model) {
		...
		String contentPW = contentService.readContent(contentDataVO).getContentDataVO().getB_pw();

		if (contentDataVO.getB_isHidden().equals("1")) {
			if (contentDataVO.getB_pw().equals(contentPW)) {
		...
		} else if (contentDataVO.getB_isHidden().equals("0")) {
	...
	public String updateContentForm(ContentDataVO contentDataVO, Model model) { // parameter에 c_idx의 정보가 아직 담겨 있음 (
