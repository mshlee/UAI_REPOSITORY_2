	...
	<div id="wrapper">
		...
		<div id="pages">
			<c:if test="${pageVO.startOfPageGroup != 1}">
				<a
					href="readWishlistPage?changePage=previous&nowPage=${pageVO.nowPage }">이전</a>
			</c:if>
			<c:forEach var="i" begin="${pageVO.startOfPageGroup }"
				...
				<c:choose>
					...
					<c:otherwise>
						<div id="page">
							<a href="./readWishlistPage?nowPage=${i }">${i }</a>
						</div>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:if test="${pageVO.endOfPageGroup != pageVO.maxPage}">
				<a
					href="readWishlistPage?changePage=next&nowPage=${pageVO.nowPage }">다음</a>
			</c:if>
		</div>
	</div>
</body>
